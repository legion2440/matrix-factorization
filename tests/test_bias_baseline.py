...
def test_old_baseline_module_no_longer_exists():
    root = Path(__file__).resolve().parents[1]
    old_module = root / "models" / ("baseline" + "_cf.py")
...
def test_predict_pairs_uses_global_user_and_item_bias_formula():
    model = BiasBaselineModel(n_users=2, n_items=2)
...
def test_baseline_clipping_and_invalid_indices():
    model = BiasBaselineModel(n_users=1, n_items=2)
    model.global_mean = 3.0
...
def test_bias_baseline_tuning_orchestration_does_not_accept_test_data(monkeypatch):
    parameters = inspect.signature(run_pipeline._tune_bias_baseline).parameters
    ...
    monkeypatch.setattr(
        run_pipeline, "BiasBaselineModel", RecordingBiasBaseline
    )
    arrays = (
    ...
    run_pipeline._fit_final_bias_baseline(arrays, 1, 1, best)
