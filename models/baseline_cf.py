...
class BaselineCFModel:
    ...
    def fit(
        ...
        if not np.isfinite(self.user_bias).all() or not np.isfinite(
            ...
            raise FloatingPointError("Baseline CF diverged to non-finite biases")
        return self

    def _check_fitted(self) -> None:
        if self.user_bias is None or self.item_bias is None:
            raise RuntimeError("BaselineCFModel has not been fitted")
